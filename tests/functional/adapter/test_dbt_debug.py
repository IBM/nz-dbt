...
class BaseDebugNetezza(BaseDebugPostgres):
...
class TestDebugNetezza(BaseDebugNetezza):
    pass
...
class TestDebugInvalidProjectNetezza(BaseDebugInvalidProject):
    def test_empty_project(self, project):
        with open("dbt_project.yml", "w") as f:  # noqa: F841
            pass
        print(f'Inside Netezza')
        run_dbt(["debug", "--profile", "test"], expect_pass=False)
        print(f'After dbt run debug --profile test')
        splitout = self.capsys.readouterr().out.split("\n")
        print(f"the value of splitout : {splitout}")
